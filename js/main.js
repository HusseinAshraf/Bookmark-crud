...
function displayBookmark(list) {
    ...
    for (var i = 0; i < list.length; i++) {
        ...
        <td>${i + 1}</td>
        <td>${list[i].name}</td>
        <td><a href="${list[i].url}" target="_blank" class="btn btn-visit text-decoration-none"><i class="fa-solid fa-eye pe-2"></i> Visit</a></td>
...
function searchBookmark(term) {
  ...
  for (var i = 0; i < allBookmark.length; i++) {
    if (allBookmark[i].name.toLowerCase().includes(term.toLowerCase()) === true) {
